...
│   ├── tricks/         ← Trick Library (ท่าบิน freestyle)
│   ├── compatibility/  ← Parts Compatibility (มาตรฐานชิ้นส่วน)
...
│   └── tricks.json     ← เพิ่ม/แก้ท่าบิน (Trick Library)
│   └── compatibility.json ← เพิ่ม/แก้ข้อมูลอ้างอิงชิ้นส่วน
├── scripts/
...
> Trick Library และ Parts Compatibility ทำเสร็จแล้ว (ดูใน `app/tricks`,
> `app/compatibility`) — เหลือ Community Presets เป็นตัวถัดไปที่ยัง priority
> อยู่ แต่ต้องมี backend (Supabase) จริงถึงจะทำได้ ไม่ใช่แค่ static JSON
